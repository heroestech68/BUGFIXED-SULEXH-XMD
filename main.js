...
async function handleMessages(sock, messageUpdate, printLog) {
    try {
        ...
        if (userMessage.startsWith('.autotyping')) {
            ...
            const mode = userMessage.includes('on') ? 'typing' : 'unavailable';
            await setPresence(mode, sock, chatId);
            await sock.sendMessage(chatId, { text: mode === 'typing' ? "✅ Typing presence enabled" : "❌ Typing presence disabled" }, { quoted: message });
            return;
        }

        if (userMessage.startsWith('.autorecording')) {
            if (!senderIsOwnerOrSudo) return;
            const mode = userMessage.includes('on') ? 'recording' : 'unavailable';
            await setPresence(mode, sock, chatId);
            await sock.sendMessage(chatId, { text: mode === 'recording' ? "✅ Recording presence enabled" : "❌ Recording presence disabled" }, { quoted: message });
            return;
        }

        if (userMessage.startsWith('.alwaysonline')) {
            if (!senderIsOwnerOrSudo) return;
            const mode = userMessage.includes('on') ? 'online' : 'unavailable';
            await setPresence(mode, sock, chatId);
            await sock.sendMessage(chatId, { text: mode === 'online' ? "✅ Online presence enabled" : "❌ Online presence disabled" }, { quoted: message });
            return;
        }
        // =====================
        ...
        // ------------------
        // REMAINS UNCHANGED
        // All your previous command logic is untouched!
        // ------------------
        ...
        switch (true) {
            ...
            // Keep your existing case logic as-is.
            // Only the presence command section and presence helper function have been fixed!
